...
async function run() {
  try {
    console.log("--- ASSOCIATE PERMISSIONS CHECK ---");
    ...
    if (errAllRp) {
      ...
      console.log("Associate permissions from rol_permisos:");
      const asociadoPerms = allRp.filter(rp => rp.roles?.nombre?.toLowerCase() === 'asociado');
      asociadoPerms.forEach(rp => {
