...
-- (asociados se crea antes que usuarios para poder referenciarla)
-- ============================================================
...
-- ── ASOCIADOS ──────────────────────────────────────────────
-- Se crea antes que usuarios para evitar dependencia circular
...
-- ── USUARIOS ───────────────────────────────────────────────
-- Extiende auth.users; asociado_id vincula al asociado de la cooperativa
...
-- ── AHORRO PERMANENTE ──────────────────────────────────────
CREATE TABLE ahorro_permanente (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE OR REPLACE TRIGGER trg_ahorro_permanente_updated_at
  BEFORE UPDATE ON ahorro_permanente
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

CREATE INDEX idx_ahorro_perm_asociado ON ahorro_permanente(asociado_id);
CREATE INDEX idx_ahorro_perm_estado   ON ahorro_permanente(estado);


-- ── MOVIMIENTOS AHORRO PERMANENTE ─────────────────────────
CREATE TABLE movimientos_ahorro_permanente (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ahorro_id        UUID NOT NULL REFERENCES ahorro_permanente(id) ON DELETE CASCADE,
  asociado_id      UUID NOT NULL REFERENCES asociados(id) ON DELETE CASCADE,
  tipo_movimiento  VARCHAR(20) NOT NULL
                   CHECK (tipo_movimiento IN ('Aporte','Retiro','Ajuste')),
...
CREATE INDEX idx_mov_ah_perm_asociado ON movimientos_ahorro_permanente(asociado_id);
CREATE INDEX idx_mov_ah_perm_fecha    ON movimientos_ahorro_permanente(fecha_movimiento);


-- ── AHORRO VOLUNTARIO ─────────────────────────────────────
CREATE TABLE ahorro_voluntario (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  asociado_id      UUID NOT NULL REFERENCES asociados(id) ON DELETE CASCADE,
  cuota_mensual    DECIMAL(12,2) NOT NULL DEFAULT 0,
  monto_ahorrado   DECIMAL(12,2) NOT NULL DEFAULT 0,
...
CREATE OR REPLACE TRIGGER trg_ahorro_voluntario_updated_at
  BEFORE UPDATE ON ahorro_voluntario
  FOR EACH ROW EXECUTE FUNCTION set_updated_at();

CREATE INDEX idx_ahorro_vol_asociado ON ahorro_voluntario(asociado_id);


-- ── MOVIMIENTOS AHORRO VOLUNTARIO ─────────────────────────
CREATE TABLE movimientos_ahorro_voluntario (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ahorro_id        UUID NOT NULL REFERENCES ahorro_voluntario(id) ON DELETE CASCADE,
  asociado_id      UUID NOT NULL REFERENCES asociados(id) ON DELETE CASCADE,
  tipo_movimiento  VARCHAR(20) NOT NULL
                   CHECK (tipo_movimiento IN ('Depósito','Retiro','Interés','Ajuste')),
  monto            DECIMAL(12,2) NOT NULL,
  saldo_anterior   DECIMAL(12,2) NOT NULL,
  saldo_nuevo      DECIMAL(12,2) NOT NULL,
  fecha_movimiento DATE NOT NULL DEFAULT CURRENT_DATE,
...
CREATE INDEX idx_mov_ah_vol_asociado ON movimientos_ahorro_voluntario(asociado_id);
...
-- ── SOLICITUDES CRÉDITO (Comité evaluador) ────────────────
...
CREATE INDEX idx_solicitudes_credito  ON solicitudes_credito(credito_id);
CREATE INDEX idx_solicitudes_asociado ON solicitudes_credito(asociado_id);
...
-- Actualizar saldo ahorro permanente del asociado
CREATE OR REPLACE FUNCTION actualizar_saldo_ahorro_permanente()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE asociados
  SET saldo_ahorro_permanente = NEW.saldo_nuevo, updated_at = NOW()
  WHERE id = NEW.asociado_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trg_saldo_ah_permanente
  AFTER INSERT ON movimientos_ahorro_permanente
  FOR EACH ROW EXECUTE FUNCTION actualizar_saldo_ahorro_permanente();


-- Actualizar saldo ahorro voluntario del asociado
CREATE OR REPLACE FUNCTION actualizar_saldo_ahorro_voluntario()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE asociados
  SET saldo_ahorro_voluntario = NEW.saldo_nuevo, updated_at = NOW()
  WHERE id = NEW.asociado_id;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trg_saldo_ah_voluntario
  AFTER INSERT ON movimientos_ahorro_voluntario
  FOR EACH ROW EXECUTE FUNCTION actualizar_saldo_ahorro_voluntario();
...
-- Crear solicitud en comité al insertar crédito pendiente
CREATE OR REPLACE FUNCTION crear_solicitud_credito()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.estado = 'pendiente' THEN
    INSERT INTO solicitudes_credito (credito_id, asociado_id, fecha_solicitud)
    VALUES (NEW.id, NEW.asociado_id, CURRENT_DATE);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE TRIGGER trg_crear_solicitud
  AFTER INSERT ON creditos
  FOR EACH ROW EXECUTE FUNCTION crear_solicitud_credito();


-- Sincronizar estado de crédito cuando el comité decide
CREATE OR REPLACE FUNCTION sincronizar_estado_credito()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.estado IN ('aprobada','rechazada') AND OLD.estado = 'pendiente' THEN
    UPDATE creditos
    SET estado           = CASE NEW.estado WHEN 'aprobada' THEN 'aprobado' ELSE 'rechazado' END,
        aprobado_por     = NEW.evaluado_por,
...
CREATE OR REPLACE TRIGGER trg_sincronizar_credito
  AFTER UPDATE ON solicitudes_credito
  FOR EACH ROW EXECUTE FUNCTION sincronizar_estado_credito();
...
ALTER TABLE ahorro_permanente              ENABLE ROW LEVEL SECURITY;
ALTER TABLE ahorro_voluntario              ENABLE ROW LEVEL SECURITY;
ALTER TABLE movimientos_ahorro_permanente  ENABLE ROW LEVEL SECURITY;
ALTER TABLE movimientos_ahorro_voluntario  ENABLE ROW LEVEL SECURITY;
...
-- ── AHORROS ────────────────────────────────────────────────
CREATE POLICY "ahorro_perm_select" ON ahorro_permanente
  FOR SELECT TO authenticated
  USING (get_user_role() = 'admin' OR asociado_id = get_asociado_id());

CREATE POLICY "ahorro_perm_write" ON ahorro_permanente
  FOR ALL TO authenticated
  USING (get_user_role() = 'admin')
  WITH CHECK (get_user_role() = 'admin');

CREATE POLICY "ahorro_vol_select" ON ahorro_voluntario
  FOR SELECT TO authenticated
  USING (get_user_role() = 'admin' OR asociado_id = get_asociado_id());

CREATE POLICY "ahorro_vol_write" ON ahorro_voluntario
  FOR ALL TO authenticated
  USING (get_user_role() = 'admin')
  WITH CHECK (get_user_role() = 'admin');
...
VALUES
  (
    ...
    '{"usuarios":["crear","leer","actualizar","eliminar"],"asociados":["crear","leer","actualizar","eliminar"],"ahorros":["crear","leer","actualizar","eliminar"],"creditos":["crear","leer","actualizar","aprobar","rechazar"],"reportes":["generar","exportar"]}',
    TRUE, TRUE
  ),
  (
    'asociado',
    'Acceso limitado a consultas personales',
    '{"perfil":["leer","actualizar"],"ahorros_propios":["leer"],"creditos_propios":["leer","solicitar"],"pedidos":["crear","leer"],"eventos":["leer"]}',
    TRUE, TRUE
  )
